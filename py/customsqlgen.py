...
def journal_books(**arg):
    ...
    (
        ...
        (
            ...
            (
                SELECT b0.bid, b0.bname, b0.sdate, b0.author, b0.descr, GROUP_CONCAT(g.gname SEPARATOR ', ' ) genre, b0.is_deleted FROM
                (
                    SELECT b.bid, b.bname, b.sdate, GROUP_CONCAT(a.aname SEPARATOR ', ' ) author , b.descr, b.is_deleted FROM ({0}) b
                ...
                LEFT OUTER JOIN (SELECT DISTINCT gnid, bid, gname FROM gnid_bid
                            INNER JOIN genres  USING(gnid)) g ON b0.bid = g.bid
                GROUP BY b0.bid
            ) b1
...
def journal_examples(**arg):
    ...
    LEFT OUTER JOIN
    (
        SELECT b0.bid, b0.bname, b0.sdate, b0.author, b0.descr, GROUP_CONCAT(g.gname SEPARATOR ', ' ) genre FROM
        (
            SELECT b.bid, b.bname, b.sdate, GROUP_CONCAT(a.aname SEPARATOR ', ' ) author , descr FROM books b
                LEFT OUTER JOIN (SELECT DISTINCT aid, bid, aname FROM aid_bid
                    INNER JOIN authors  USING(aid)) a ON b.bid = a.bid
            GROUP BY b.bid
        ) b0
...
def journal_acts(**arg):
    pattern ='''
        ...
        (
            ...
            LEFT OUTER JOIN
            (
                SELECT b0.bid, b0.bname,  b0.author, GROUP_CONCAT(g.gname SEPARATOR ", " ) genre FROM
                (
                    SELECT b.bid, b.bname,  GROUP_CONCAT(a.aname SEPARATOR ", "  ) author
                        FROM
                        (
